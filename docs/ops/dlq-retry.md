# 사후 처리 큐(DLQ) 재처리 안내서 (상세)

write-behind 워커(`wb_worker`)가 실패한 이벤트를 DLQ로 보내면, `wb_dlq_replayer`가 재처리한다. 본 문서는 구조, 설정, 모니터링, 트러블슈팅을 모두 다룬다.
...
| 증상 | 조치 |
| --- | --- |
| DLQ backlog 급증 | ① 최근 배포 롤백(rollback) ② 오류 로그에서 SQL/네트워크 원인 확인 |
| dead 스트림 증가 | 데이터 손상 여부 확인 후 수동 패치(patch) 또는 삭제 |
| replayer crash | `WB_RETRY_MAX` 값을 임시로 줄이고, 문제가 되는 이벤트 ID 추적 |

## 6. FAQ
- **Q. DLQ를 비우지 않고도 서버를 계속 운영할 수 있나?** -> 가능하지만 데이터 정합성을 위해 24시간 내 처리를 권장한다.
- **Q. DLQ 항목을 수동 수정해도 되나?** -> payload가 JSON이면 수동 패치(patch)가 가능하나, 변경 이력을 남기고 재처리 결과를 꼭 확인한다.
