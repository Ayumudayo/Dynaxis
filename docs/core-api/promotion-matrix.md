...
- `stay transitional`: plugin/Lua extensibility governance seam
...
- FPS public capability work should promote a narrower transport/runtime contract above the current internal session/RUDP internals rather than exposing these implementation headers directly.
  - current promoted runtime slice: `server/core/fps/runtime.hpp`
  - current promoted transport-policy slice: `server/core/fps/direct_delivery.hpp`
- Extensibility remains the only intentionally public non-stable seam in the current engine boundary.
