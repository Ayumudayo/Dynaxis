...
- `EngineRuntime::clear_global_services()`는 legacy `ServiceRegistry` compatibility bridge 정리 경로를 공통화합니다.
- `EngineContext`는 앱별 bootstrap이 core primitive를 지역적으로 등록하는 instance-scoped typed registry입니다.
