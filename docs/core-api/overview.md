...
| Realtime Capability | `[Stable]` | `server/core/realtime/direct_bind.hpp`, `server/core/realtime/direct_delivery.hpp`, `server/core/realtime/simulation_phase.hpp`, `server/core/realtime/transport_quality.hpp`, `server/core/realtime/transport_policy.hpp`, `server/core/realtime/runtime.hpp` | fixed-step runtime, bind payload contract, deterministic simulation phase vocabulary, rollout/canary/allowlist policy, direct delivery route policy, sequenced UDP quality tracking, coarse interest management, rewind/history lookup |
...
| Networking | `[Stable]+[Internal]` | `server/core/net/hive.hpp`, `server/core/net/listener.hpp`, `server/core/net/connection.hpp`, `server/core/net/transport_router.hpp`, `server/core/net/dispatcher.hpp` | 이벤트 루프 수명주기, bounded transport substrate, stable higher-level transport/session routing seam, 그리고 기존 packet dispatcher contract |
...
- higher-level transport/session public routing seam의 canonical path/namespace는 `server/core/net/transport_router.hpp`와 `server::core::net::{ITransportSession, TransportRouter}`입니다.
...
- Runtime Host surface는 app bootstrap 공통화를 위한 기본 composition seam입니다.
  ...
  - `EngineBuilder::register_module()` / `EngineRuntime::register_module()` / `start_modules()`는 runtime-owned orchestration module registration, ordered startup, shutdown hook attachment를 위한 stable composition hook입니다.
  ...
  - `EngineRuntime::snapshot()`은 lifecycle/readiness/stop/context-service-count/compatibility-bridge-count와 module/watchdog aggregate counts를 인스턴스 단위로 읽는 canonical embeddability view입니다.
  - `EngineRuntime::module_snapshot()`은 runtime-owned module started/watchdog detail을 registration order 기준으로 읽는 canonical operator-facing detail surface입니다.
...
- Concurrency surface는 `TaskScheduler`, `JobQueue`, `ThreadManager`의 caller-owned execution 모델을 stable contract로 유지한다.
  - `TaskScheduler`는 호출자가 `poll()` 루프와 실행 주기를 직접 소유한다.
  - `TaskScheduler`의 stable control surface는 cancel token/group, validator-gated one-shot/repeat scheduling, repeat context, reschedule, repeat-policy update를 포함한다.
  - `JobQueue`와 `ThreadManager`는 명시적 종료 순서를 전제로 한다.
  - 내부 큐 구성요소는 런타임 배선용이며 공개 예제 surface로 취급하지 않는다.
- Metrics/Lifecycle surface는 `EngineRuntime`의 instance-scoped lifecycle snapshot과 `runtime_metrics`의 process-wide liveness/watchdog/detailed-telemetry aggregate를 함께 stable contract로 유지한다.
