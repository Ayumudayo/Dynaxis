...
#### 2.3.2 Core RUDP (계획, 기본 OFF)

아래 키는 **설계 고정용 계약**이며, 단계적 구현 완료 전까지는 활성 경로로 간주하지 않는다.

| 변수/옵션 | 기본값 | 설명 |
| --- | --- | --- |
| `KNIGHTS_ENABLE_CORE_RUDP` (CMake option) | `OFF` | core RUDP 엔진 컴파일 게이트(기본 비활성) |
...
| `GATEWAY_RUDP_HANDSHAKE_TIMEOUT_MS` | `1500` | RUDP HELLO/ACK 완료 타임아웃(ms, 계획값) |
| `GATEWAY_RUDP_IDLE_TIMEOUT_MS` | `10000` | 유휴 상태 timeout(ms, 계획값) |
| `GATEWAY_RUDP_ACK_DELAY_MS` | `10` | delayed ACK 기본 지연(ms, 계획값) |
| `GATEWAY_RUDP_MAX_INFLIGHT_PACKETS` | `256` | 세션별 최대 in-flight 패킷 수(계획값) |
| `GATEWAY_RUDP_MAX_INFLIGHT_BYTES` | `262144` | 세션별 최대 in-flight 바이트(계획값) |
| `GATEWAY_RUDP_MTU_PAYLOAD_BYTES` | `1200` | datagram payload 상한(계획값) |
| `GATEWAY_RUDP_RTO_MIN_MS` | `50` | 재전송 RTO 하한(ms, 계획값) |
| `GATEWAY_RUDP_RTO_MAX_MS` | `2000` | 재전송 RTO 상한(ms, 계획값) |
