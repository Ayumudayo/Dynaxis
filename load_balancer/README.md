...
- 세션 디렉터리: `LB_SESSION_TTL` 동안 `client_id`→백엔드 매핑을 Redis(`gateway/session/<client>`)에 저장해 sticky routing을 유지합니다.
- 백엔드 헬스 게이팅: `LB_BACKEND_FAILURE_THRESHOLD`를 초과하면 `LB_BACKEND_COOLDOWN` 동안 백엔드를 제외합니다.
- Consistent hashing: 매핑이 없으면 Consistent Hash ring으로 백엔드를 선택하고 실패 시 라운드로빈으로 폴백합니다.
