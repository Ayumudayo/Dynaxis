...
TEST(MmorpgMigrationContractTest, ReportsReadyToResumeWhenTargetWorldOwnerIsReadyAndSourceDrains) {
    server::core::worlds::ObservedWorldMigrationWorld source_world{
        .world_id = "starter-a",
    ...
    server::core::worlds::ObservedWorldMigrationWorld target_world{
        .world_id = "starter-b",
    ...
    const auto status =
        server::core::worlds::evaluate_world_migration(source_world, envelope, target_world);
    EXPECT_EQ(status.phase, server::core::worlds::WorldMigrationPhase::kReadyToResume);
...
TEST(MmorpgMigrationContractTest, ReportsAwaitingDrainWhenTargetIsReadyButSourceIsNotDraining) {
    ...
    const auto status =
        server::core::worlds::evaluate_world_migration(source_world, envelope, target_world);
    EXPECT_EQ(status.phase, server::core::worlds::WorldMigrationPhase::kAwaitingSourceDrain);
}
...
TEST(MmorpgMigrationContractTest, ReportsMissingTargetWorldOrOwnerStates) {
    ...
    const auto missing_world =
        server::core::worlds::evaluate_world_migration(source_world, envelope, std::nullopt);
    EXPECT_EQ(missing_world.phase, server::core::worlds::WorldMigrationPhase::kTargetWorldMissing);

    server::core::worlds::ObservedWorldMigrationWorld missing_owner_world{
        .world_id = "starter-b",
        .current_owner_instance_id = "server-3",
        .instances = {{.instance_id = "server-3", .ready = true}},
    };
    const auto missing_owner =
        server::core::worlds::evaluate_world_migration(source_world, envelope, missing_owner_world);
    EXPECT_EQ(missing_owner.phase, server::core::worlds::WorldMigrationPhase::kTargetOwnerMissing);

    server::core::worlds::ObservedWorldMigrationWorld unready_owner_world{
        .world_id = "starter-b",
        .current_owner_instance_id = "server-2",
        .instances = {{.instance_id = "server-2", .ready = false}},
    };
    const auto unready_owner =
        server::core::worlds::evaluate_world_migration(source_world, envelope, unready_owner_world);
    EXPECT_EQ(unready_owner.phase, server::core::worlds::WorldMigrationPhase::kTargetOwnerNotReady);
}
