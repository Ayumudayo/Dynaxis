...
## 참고
- `server_app`은 (실험) chat hook 플러그인을 사용할 수 있다. 기본 스택은 `CHAT_HOOK_PLUGINS_DIR=/app/plugins`를 사용하며, 호스트 `docker/stack/plugins`를 read-only 마운트한다. 디렉터리가 비어 있으면 `CHAT_HOOK_FALLBACK_PLUGINS_DIR=/app/plugins_builtin`으로 이미지 내 샘플 플러그인을 로드한다. (`server/README.md` 참고)
- Lua cold-hook 샘플 스크립트는 이미지에 `/app/scripts_builtin`으로 포함되며, 스택은 `docker/stack/scripts/*.lua`를 `/app/scripts`로 read-only 마운트해 우선 로드한다. `/app/scripts`가 비어 있거나 읽기 실패면 `LUA_FALLBACK_SCRIPTS_DIR=/app/scripts_builtin`으로 fallback한다.
