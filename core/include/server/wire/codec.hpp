...
inline std::vector<std::uint8_t> Encode(const server::wire::v1::WhisperResult& m) { std::string bytes; m.SerializeToString(&bytes); return std::vector<std::uint8_t>(bytes.begin(), bytes.end()); }
inline std::vector<std::uint8_t> Encode(const server::wire::v1::WhisperNotice& m) { std::string bytes; m.SerializeToString(&bytes); return std::vector<std::uint8_t>(bytes.begin(), bytes.end()); }
