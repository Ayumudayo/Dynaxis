...
std::vector<ReplicationUpdate> WorldRuntime::tick() {
    ...
    ISimulationPhaseObserver* observer = nullptr;

    {
        std::lock_guard<std::mutex> lock(mu_);
        observer = simulation_phase_observer_;
        outbound.reserve(viewers_.size());

        const auto capture_phase = [&](SimulationPhase phase, std::size_t replication_update_count = 0u) {
            if (!observer) {
                return;
            }
            phase_events.emplace_back(phase, SimulationPhaseContext{
                .server_tick = server_tick_,
                .actor_count = actors_.size(),
                .viewer_count = viewers_.size(),
                .staged_input_count = staged_inputs_.size(),
                .replication_update_count = replication_update_count,
            });
        };

        ++server_tick_;
        ++tick_total_;
        capture_phase(SimulationPhase::kTickBegin);
        ...
        capture_phase(SimulationPhase::kInputsApplied);
        ...
        capture_phase(SimulationPhase::kActorsAdvanced);
        ...
        capture_phase(SimulationPhase::kReplicationComputed, outbound.size());

        for (auto& [actor_id, actor] : actors_) {
            actor.dirty = false;
        }
        removed_actor_ids_.clear();
        capture_phase(SimulationPhase::kTickEnd, outbound.size());
    }
