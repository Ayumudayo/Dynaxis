...
- `tools/admin_app/main.cpp` + `tools/admin_app/admin_ui.html`: read-only admin control-plane (`/metrics`, `/healthz`, `/readyz`, `/admin`, `/api/v1/auth/context`, `/api/v1/overview`, `/api/v1/instances`, `/api/v1/sessions/{client_id}`, `/api/v1/worker/write-behind`, `/api/v1/metrics/links`).
